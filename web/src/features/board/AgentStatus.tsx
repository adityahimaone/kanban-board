...
export function useElapsed(startedAt?: number | null, active = true) {
  ...
  const seconds = Math.max(0, (now - startedAt * 1000) / 1000)
...
export function RunningIndicator({ startedAt, compact = false }: { startedAt?: number | null; compact?: boolean }) {
...
export function AgentTaskStatus({ task, events }: { task: Task; events: TaskEvent[] }) {
  const rows = useMemo(() => {
    ...
    return relevant.map((event, index) => ({
      event,
      state: index === 0 && task.status === "running" ? "running" : event.kind === "completed" ? "done" : event.kind.includes("fail") || event.kind === "blocked" ? "failed" : "done",
    }))
  }, [events, task.status])
    ...
    <section className="rounded-lg border border-sky-500/30 bg-sky-500/5 p-2.5" aria-label="Live agent task status">
      <div className="flex items-center justify-between gap-2">
        <div className="flex min-w-0 items-center gap-2">
          {task.status === "running" ? <LoaderGrid /> : <span className={`size-2 rounded-full ${task.status === "done" ? "bg-emerald-400" : "bg-neutral-500"}`} />}
          <h3 className="truncate text-[10px] font-semibold uppercase tracking-wider text-sky-300">Live agent status</h3>
        </div>
        {task.status === "running" && <RunningIndicator startedAt={task.started_at} compact />}
      </div>
      {rows.length > 0 ? (
        <div className="mt-2 flex flex-col gap-1.5">
          {rows.map(({ event, state }) => (
            <div key={event.id} className="flex items-center gap-2 text-[11px]">
              <span className={`size-1.5 rounded-full ${state === "running" ? "bg-sky-400" : state === "failed" ? "bg-red-400" : "bg-emerald-400"}`} />
