      ...
      <div className="absolute left-0 top-0 z-10 origin-top-left" style={{ width: GRAPH_W, height: GRAPH_H, transform: `translate(${actualView.x}px, ${actualView.y}px) scale(${actualView.scale})` }}>
        <svg className="pointer-events-none absolute inset-0" width={GRAPH_W} height={GRAPH_H}>
          ...
          {edgePaths.map((e) => <g key={`${e.from}-${e.to}`}>
            <path d={e.path} fill="none" stroke={e.color} strokeWidth={e.active ? 12 : 7} opacity={e.active ? .45 : .3} filter="url(#flow-edge-blur)" />
            <path d={e.path} fill="none" stroke={e.color} strokeWidth={e.active ? 3.5 : 2.5} opacity={e.active ? 1 : .8} strokeLinecap="round" />
          </g>)}
        </svg>
        {visibleTasks.slice(0, 24).flatMap((t, i) => { const chain = channelChain(t); return chain.slice(0, -1).flatMap((from, hop) => { const to = chain[hop + 1]; const path = anchor(from, to, positions)[2]; return [0, 1, 2, 3].map((dot) => <TravelingDot key={`${t.task_id}-${from}-${to}-${dot}`} taskId={`${t.task_id}-${i}`} pathD={path} pathLen={pathLength(path)} phaseRatio={(i + hop + dot / 4) / Math.max(1, visibleTasks.length + 4)} active />) }) })}
        {edgePaths.flatMap((edge, i) => [0, 1, 2].map((phase) => <TravelingDot key={`idle-${edge.from}-${edge.to}-${phase}`} taskId={`idle-${i}`} pathD={edge.path} pathLen={pathLength(edge.path)} phaseRatio={(i * .17 + phase / 3) % 1} idle />))}
