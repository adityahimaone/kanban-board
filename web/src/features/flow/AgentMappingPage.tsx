...
export default function AgentMappingPage() {
  ...
  return <div className="relative flex min-h-0 flex-1 flex-col bg-bg text-ink">
    <div className="flex min-h-16 shrink-0 flex-wrap items-center gap-3 border-b border-line bg-surface px-4 py-3">
      ...
      <div className="relative w-full sm:w-48"><Search className="absolute left-2 top-1/2 size-3 -translate-y-1/2 text-ink-3" /><input aria-label="Search active tasks" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search tasks" className="h-8 w-full rounded-md border border-line bg-inset pl-7 pr-2 text-xs text-ink outline-none placeholder:text-ink-4 focus:border-accent/70 focus:ring-1 focus:ring-accent/20" /></div>
      <select aria-label="Filter task stage" value={stage} onChange={(e) => setStage(e.target.value as FlowStage | "all")} className="h-8 rounded-md border border-line bg-inset px-2 text-xs text-ink outline-none focus:border-accent/70 focus:ring-1 focus:ring-accent/20"><option value="all">All stages</option><option value="dispatched">Dispatched</option><option value="running">Running</option><option value="done">Done</option><option value="failed">Failed</option></select>
      <span className="border-l border-line pl-3 font-mono text-[10px] text-ink-3">{activeCount} active</span><span className="flex items-center gap-1.5 font-mono text-[10px] text-green"><i className="size-1.5 rounded-full bg-current" /> connected</span>
    </div>
    {isError && <div className="border-b border-rose/30 bg-red-tint px-4 py-2 text-xs text-rose">Flow Map could not load /api/flow/active.</div>}
