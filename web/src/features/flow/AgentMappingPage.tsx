...
const COLORS: Record<FlowNodeId, string> = { kanban: "#8f83ff", orchestrator: "#10e0dd", dispatcher: "#f59e0b", memory: "#6366f1", "node-agent-server": "#f2c94c", tailscale: "#64b7ff", mac: "#57d18d", windows: "#57d18d", review: "#22c55e" }
...
export default function AgentMappingPage() {
  ...
  return <div className="relative flex min-h-0 flex-1 flex-col bg-[#0b0e14] text-[#e6e9ef]">
    <div className="flex min-h-16 shrink-0 flex-wrap items-center gap-3 border-b border-[#1e2430] bg-[#11151f] px-4 py-3">
      <div className="mr-auto flex items-center gap-2"><Network className="size-4 text-[#10e0dd]" /><div><h1 className="text-sm font-semibold tracking-tight">Flow Map</h1><p className="text-[10px] text-[#8b93a7]">Live task routing and execution map</p></div></div>
      <div className="relative w-full sm:w-48"><Search className="absolute left-2 top-1/2 size-3 -translate-y-1/2 text-[#8b93a7]" /><input aria-label="Search active tasks" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search tasks" className="h-8 w-full rounded-md border border-[#1e2430] bg-[#0b0e14] pl-7 pr-2 text-xs outline-none placeholder:text-[#687187] focus:border-[#10e0dd]/70 focus:ring-1 focus:ring-[#10e0dd]/20" /></div>
      <select aria-label="Filter task stage" value={stage} onChange={(e) => setStage(e.target.value as FlowStage | "all")} className="h-8 rounded-md border border-[#1e2430] bg-[#0b0e14] px-2 text-xs outline-none focus:border-[#10e0dd]/70 focus:ring-1 focus:ring-[#10e0dd]/20"><option value="all">All stages</option><option value="dispatched">Dispatched</option><option value="running">Running</option><option value="done">Done</option><option value="failed">Failed</option></select>
      <span className="border-l border-[#1e2430] pl-3 font-mono text-[10px] text-[#8b93a7]">{activeCount} active</span><span className="flex items-center gap-1.5 font-mono text-[10px] text-emerald-300"><i className="size-1.5 rounded-full bg-current" /> connected</span>
    </div>
    ...
    <div ref={canvasRef} className="relative min-h-0 flex-1 overflow-hidden select-none" onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerUp} onPointerCancel={onPointerUp} onWheel={(e) => { if (e.ctrlKey || e.metaKey) { e.preventDefault(); zoomAt(e.deltaY < 0 ? 1.12 : .89) } }} style={{ backgroundImage: "radial-gradient(rgba(16,224,221,.12) 1px, transparent 1px)", backgroundSize: "18px 18px" }}>
      <div className="absolute left-0 top-0 origin-top-left" style={{ width: GRAPH_W, height: GRAPH_H, transform: `translate(${actualView.x}px, ${actualView.y}px) scale(${actualView.scale})` }}>
        ...
        {visibleTasks.slice(0, 24).map((t, i) => { const chain = t.stage === "dispatched" ? ["kanban", "dispatcher", "node-agent-server"] : t.stage === "running" && (t.node_id === "mac" || t.node_id === "windows") ? ["dispatcher", "node-agent-server", "tailscale", t.node_id] : (t.stage === "done" || t.stage === "failed") && (t.node_id === "mac" || t.node_id === "windows") ? [t.node_id, "review", "kanban"] : ["kanban", "dispatcher", "node-agent-server"]; const d = chain.map((id, n) => n < chain.length - 1 ? anchor(id as FlowNodeId, chain[n + 1] as FlowNodeId, positions)[2] : "").join(" "); return <TravelingDot key={t.task_id} taskId={`${t.task_id}-${i}`} pathD={d} pathLen={pathLength(d)} phaseRatio={i / Math.max(1, visibleTasks.length)} /> })}
        ...
        {NODES.map((node) => { const Icon = ICON[node.id], task = stageFor(node.id, visibleTasks), color = task ? STAGE_COLOR[task.stage] : COLORS[node.id]; return <button key={node.id} type="button" onPointerDown={(e) => onNodePointerDown(e, node.id)} onPointerMove={onNodePointerMove} onPointerUp={(e) => onNodePointerUp(e, node.id)} onPointerCancel={() => { nodeDragRef.current = null }} className={`map-node absolute flex h-[52px] w-[188px] cursor-grab items-center gap-2 rounded-xl border bg-[#11151f] px-3 text-left shadow-[0_8px_24px_rgba(0,0,0,.18)] transition-colors duration-150 hover:bg-[#161b27] active:cursor-grabbing focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#10e0dd] ${!visibleTasks.length ? "map-idle-card" : ""} ${selected === node.id ? "ring-1" : ""}`} style={{ left: positions[node.id].x - CARD_W / 2, top: positions[node.id].y - CARD_H / 2, borderColor: selected === node.id ? color : "rgba(30,36,48,.9)", boxShadow: selected === node.id ? `0 0 0 1px ${color}, 0 8px 24px rgba(0,0,0,.22)` : undefined }}><span className="absolute inset-y-2 left-0.5 w-0.5 rounded-full" style={{ background: color }} /><Icon className="ml-1 size-3.5 shrink-0" style={{ color }} /><span className="min-w-0 flex-1"><span className="block truncate text-[11px] font-semibold">{LABEL[node.id]}</span><span className="block truncate font-mono text-[9px] text-[#8b93a7]">{task ? `${task.stage} · ${task.task_id}` : node.sub}</span></span>{countFor(node.id) > 0 && <span className="flex size-4 shrink-0 items-center justify-center rounded-full text-[9px] font-bold text-black" style={{ background: color }}>{countFor(node.id)}</span>}</button> })}
      ...
      <div onPointerDown={(e) => e.stopPropagation()} className="absolute bottom-4 left-4 flex items-center gap-1 rounded-lg border border-[#1e2430] bg-[#11151f]/95 p-1 shadow-[0_8px_24px_rgba(0,0,0,.18)]"><button aria-label="Zoom out" title="Zoom out" className="map-control" onClick={() => zoomAt(.8)}><ZoomOut className="size-3.5" /></button><span className="w-10 text-center font-mono text-[10px] text-[#8b93a7]">{Math.round(actualView.scale * 100)}%</span><button aria-label="Zoom in" title="Zoom in" className="map-control" onClick={() => zoomAt(1.2)}><ZoomIn className="size-3.5" /></button><button aria-label="Fit graph" title="Fit graph" className="map-control" onClick={() => setView(null)}><Maximize2 className="size-3.5" /></button><button aria-label={expanded ? "Exit expanded map" : "Expand map"} title={expanded ? "Exit expanded map" : "Expand map"} className="map-control" onClick={toggleExpand}>{expanded ? <Minimize2 className="size-3.5" /> : <Expand className="size-3.5" />}</button></div>
      <button type="button" aria-label="Recenter map from minimap" title="Recenter map" onPointerDown={(e) => e.stopPropagation()} onClick={() => setView({ scale: actualView.scale, x: dims.w / 2 - (GRAPH_W / 2) * actualView.scale, y: dims.h / 2 - (GRAPH_H / 2) * actualView.scale })} className="absolute bottom-4 right-4 hidden rounded-xl border border-[#1e2430] bg-[#11151f]/95 p-2 text-left shadow-[0_8px_24px_rgba(0,0,0,.22)] md:block" style={{ width: MINIMAP_W, height: MINIMAP_H }}>
        <span className="absolute inset-2 block">
          <svg viewBox={`0 0 ${GRAPH_W} ${GRAPH_H}`} preserveAspectRatio="xMidYMid meet" aria-label="Map minimap" role="img" className="h-full w-full" onClick={(e) => {
            ...
            <rect x={minimapView.left} y={minimapView.top} width={minimapView.width} height={minimapView.height} rx="8" fill="rgba(16,224,221,.08)" stroke="#10e0dd" strokeWidth="3" />
          </svg>
        </span>
