...
export function SessionMonitor({ tasks, focused, onFocus }: { tasks: FlowTask[]; focused: string | null; onFocus: (id: string | null) => void }) {
  ...
  return <section className="absolute bottom-4 left-4 z-10 w-[min(588px,calc(100%-24px))] overflow-hidden rounded-xl border border-[#1e2430] bg-[#11151f]/95 shadow-[0_8px_24px_rgba(0,0,0,.18)]" style={{ height: collapsed ? 38 : height }}>
    ...
    {!collapsed && <div className="h-[calc(100%-38px)] overflow-auto">{visible.length === 0
      ? <div className="px-3 py-4 text-center font-mono text-[10px] text-[#687187]">no active task</div>
      : <table className="w-full table-fixed text-left font-mono text-[10px]"><thead className="sticky top-0 bg-[#11151f] text-[#8b8e86]"><tr><th className="w-24 px-3 py-1.5">State</th><th className="w-[38%] px-2 py-1.5">Session</th><th className="px-2 py-1.5">Route</th></tr></thead><tbody>{visible.map((t) => { const active = focused === t.task_id; const terminal = t.stage === "done" || t.stage === "failed"; return <tr key={t.task_id} onClick={() => onFocus(active ? null : t.task_id)} className={`cursor-pointer border-t border-white/5 ${active ? "bg-white/[.05]" : "hover:bg-white/[.03]"}`}><td className="px-3 py-2"><span className="mr-1.5 inline-block size-1.5 rounded-full" style={{ background: t.stage === "failed" ? "#ef6b73" : t.stage === "done" ? "#57d18d" : t.stage === "running" ? "#8fd14f" : "#f2c94c" }} />{LABELS[t.stage]}</td><td className="min-w-0 truncate px-2 py-2 text-[#e8e8e3]" title={`${t.title || "untitled"} · ${age(t, now)}${terminal ? ` · expires ${expiry(t, now)}` : ""}`}><span style={{ color: colorForTask(t.task_id) }}>{t.task_id}</span> <span className="text-[#8b8e86]">· {t.title || "untitled"}</span></td><td className="truncate px-2 py-2 text-[#8b8e86]" title={channelPath(t.stage, t.node_id).join(" → ")}>{channelPath(t.stage, t.node_id).join(" → ")}</td></tr> })}</tbody></table>}</div>}
