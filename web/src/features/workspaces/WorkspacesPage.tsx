...
import { FolderGit2, Plus, RefreshCw, ScrollText, Trash2, Pencil, Loader2 } from "lucide-react"
...
function StatusChip({ ws }: { ws: Workspace }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-full border border-[#1e2430] bg-[#0b0e14] px-2 py-0.5 text-[11px] ${s.text}`}>
...
function WorkspaceForm({
  ...
  async function submit() {
    ...
    try {
      await onSave({ id: id.trim().toLowerCase(), name: name.trim() || id.trim(), path: path.trim(), host: host.trim(), kind })
      onClose()
      ...
      <div className="w-full max-w-md rounded-lg border border-[#1e2430] bg-[#11151f] p-4" onClick={(e) => e.stopPropagation()}>
        ...
        {!editing && (
          <>
            ...
            <input value={id} onChange={(e) => setId(e.target.value)} placeholder="mac-dev" className={`mt-1 ${field}`} />
          </>
        )}
        <label className="mt-3 block text-xs text-neutral-400">Name</label>
        <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Mac Dev" className={`mt-1 ${field}`} />
        <label className="mt-3 block text-xs text-neutral-400">Path (di host)</label>
        <input value={path} onChange={(e) => setPath(e.target.value)} placeholder="/Users/adityahimawan/Development" className={`mt-1 ${field}`} />
        <label className="mt-3 block text-xs text-neutral-400">SSH host (kosong = lokal VPS)</label>
        <input value={host} onChange={(e) => setHost(e.target.value)} placeholder="mac-tailscale" className={`mt-1 ${field}`} />
        <label className="mt-3 block text-xs text-neutral-400">Kind</label>
        <select value={kind} onChange={(e) => setKind(e.target.value)} className={`mt-1 ${field}`}>
      ...
      {workspaces.isLoading ? (
        ...
        <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2">
          {(workspaces.data ?? []).map((ws) => (
            <article key={ws.id} className="rounded-lg border border-[#1e2430] bg-[#11151f] p-3.5">
