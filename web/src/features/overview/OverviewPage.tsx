...
function UsageGauge({ icon: Icon, label, value, note, tone }: { icon: typeof Cpu; label: string; value: number; note: string; tone: string }) {
  ...
  const color = pct > 80 ? "#ef6b73" : pct > 50 ? "#f2c94c" : tone
  return <div className="rounded-xl border border-[#1e2430] bg-[#11151f] p-4"><div className="flex items-center gap-2 text-[10px] uppercase tracking-wider text-[#8b93a7]"><Icon className="size-3.5" style={{ color }} />{label}</div><div className="relative mx-auto mt-2 h-24 max-w-[180px]"><svg viewBox="0 0 110 64" className="h-full w-full" role="img" aria-label={`${label} ${pct.toFixed(1)} percent`}><path d="M13 54 A42 42 0 0 1 97 54" fill="none" stroke="#0b0e14" strokeWidth="9" strokeLinecap="round" pathLength="100" /><path d="M13 54 A42 42 0 0 1 97 54" fill="none" stroke={color} strokeWidth="9" strokeLinecap="round" pathLength="100" strokeDasharray={`${pct} ${100 - pct}`} /><circle cx="13" cy="54" r="2" fill="#687187" /><circle cx="97" cy="54" r="2" fill="#687187" /></svg><strong className="absolute inset-x-0 bottom-0 text-center font-mono text-2xl tabular-nums text-[#e6e9ef]">{pct.toFixed(1)}%</strong></div><p className="mt-1 truncate text-center text-[10px] text-[#687187]">{note}</p></div>
}
...
export default function OverviewPage() {
  ...
  return <div className="min-h-0 flex-1 overflow-y-auto bg-[#0b0e14] p-4 text-[#e6e9ef] md:p-6"><div className="mx-auto max-w-7xl"><div className="flex items-end justify-between"><div><p className="font-mono text-[10px] uppercase tracking-[.18em] text-[#10e0dd]">Hermes Runtime</p><h1 className="mt-1 text-xl font-semibold tracking-tight">Overview</h1><p className="mt-1 text-xs text-[#8b93a7]">Live system health, task throughput, and model activity.</p></div><span className="flex items-center gap-1.5 font-mono text-[10px] text-emerald-300"><i className="size-1.5 rounded-full bg-current" /> live · 5s</span></div>
    <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><UsageGauge icon={Cpu} label="CPU usage" value={data.metrics.cpu_percent} note={`${data.metrics.goroutines} goroutines · live host load`} tone="#10e0dd" /><UsageGauge icon={MemoryStick} label="Memory" value={memory} note={`${data.metrics.memory_used_mb} / ${data.metrics.memory_total_mb} MB`} tone="#a78bfa" /><Metric icon={Activity} label="Running tasks" value={String(data.running_tasks)} note={`${data.total_tasks} total tasks`} tone="#8fd14f" /><Metric icon={TriangleAlert} label="Failed tasks" value={String(data.failed_tasks)} note={`${data.completed_tasks} completed`} tone="#ef6b73" /></div>
