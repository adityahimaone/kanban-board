...
function Metric({ icon: Icon, label, value, note, tone = "var(--color-accent)" }: { icon: typeof Cpu; label: string; value: string; note: string; tone?: string }) {
  return <div className="glass-panel rounded-xl p-4"><div className="flex items-center gap-2 text-[10px] uppercase tracking-wider text-[var(--color-ink-3)]"><Icon className="size-3.5" style={{ color: tone }} />{label}</div><p className="mt-3 font-mono text-2xl font-semibold tabular-nums text-[var(--color-ink)]">{value}</p><p className="mt-1 text-[10px] text-[var(--color-ink-4)]">{note}</p></div>
}
...
function SankeyCard({ links, mode, note }: { links: FlowLink[]; mode: string; note: string }) {
  ...
  return <section className="glass-panel rounded-xl p-4"><div className="flex items-center justify-between"><div><h2 className="text-sm font-semibold">Model / provider activity</h2><p className="mt-1 text-[10px] text-[var(--color-ink-4)]">Task flow through Hermes profiles to providers and models.</p></div><span className="status-pill">{mode}</span></div>
