  ...
  return (
    <div className="flex min-h-0 flex-1 gap-3 overflow-hidden p-3" aria-hidden="true">
      {["w-72", "w-72", "w-72", "w-60"].map((width, column) => (
        <section key={column} className={`flex h-full ${width} shrink-0 flex-col rounded-xl border border-[#1e2430]/60 bg-[#11151f]/40 p-2`}>
          <div className="flex items-center justify-between border-b border-[#1e2430]/60 px-2 py-2">
            <ShimmerBar className="h-2.5 w-20" />
            <ShimmerBar className="h-4 w-5" />
          </div>
          <div className="space-y-2 py-2">
            {[0, 1, 2].slice(0, column === 3 ? 2 : 3).map((card) => (
              <div key={card} className="rounded-lg border border-[#1e2430]/60 bg-[#0b0e14]/60 p-3.5">
